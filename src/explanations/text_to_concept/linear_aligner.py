...
class LinearAligner:
    ...
    def train(
        self, ftrs1, ftrs2, epochs=6, target_variance=4.5, verbose=0, writer=None
    ) -> dict:
    ...
    def save_W(self, path_to_save: str):
        torch.save(
            {"b": self.b.detach().cpu(), "W": self.W.detach().cpu()}, path_to_save
        )
...
class LinearRegressionSolver:
    ...
    def train(
        ...
        if writer is not None:
            writer.add_scalar("MSE/Initial", init_mse, 0)
            writer.add_scalar("R2/Initial", init_r2, 0)
