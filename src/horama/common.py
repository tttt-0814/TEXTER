...
def optimization_step(
    ...
    cropped_and_resized_images = F.interpolate(
...
def optimization_step_wo_crop(
    ...
    image.retain_grad()
    # add normal and uniform noise for better robustness
    image.add_(torch.randn_like(image) * noise_level)
    image.add_((torch.rand_like(image) - 0.5) * noise_level)

    # compute the score and loss
    score = objective_function(image.unsqueeze(0))
    loss = -score
